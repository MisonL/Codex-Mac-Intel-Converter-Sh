...
function buildUpdaterReplacement() {
  return [
    ...
    "extractReleaseInfo(e,t){let n=typeof e.tag_name==`string`?e.tag_name.trim():``,r=/^v([0-9]+(?:\\.[0-9]+)+)-([A-Za-z0-9_]+)-([0-9]{8})$/.exec(n);if(!r)return G9().warning(`Latest release tag does not match expected pattern.`,{safe:{tag:n},sensitive:{}}),null;let i=r[2];if(i!==t.archLabel)return G9().info(`Latest release arch does not match current app arch.`,{safe:{tag:n,remoteArch:i,currentArch:t.archLabel},sensitive:{}}),null;let a=Array.isArray(e.assets)?e.assets:[],o=t.assetName.length>0?a.find(e=>e&&typeof e.name==`string`&&e.name===t.assetName):null;o||(o=a.find(e=>e&&typeof e.name==`string`&&e.name.endsWith(`.dmg`)&&e.name.includes(`_${t.archLabel}_`)));if(!o)return G9().warning(`Latest release is missing a matching DMG asset.`,{safe:{tag:n,arch:t.archLabel},sensitive:{}}),null;let s=typeof e.html_url==`string`?e.html_url:null,l=typeof o.browser_download_url==`string`?o.browser_download_url:null;return l==null?(G9().warning(`Latest release DMG asset is missing browser download URL.`,{safe:{tag:n,assetName:o.name??null},sensitive:{}}),null):{tag:n,version:r[1],arch:i,releaseDate:r[3],downloadUrl:l,releasePageUrl:s}}",
...
function buildModernUpdaterReplacement() {
  return [
    ...
    "extractReleaseInfo(e,t){let n=typeof e.tag_name==`string`?e.tag_name.trim():``,r=/^v([0-9]+(?:\\.[0-9]+)+)-([A-Za-z0-9_]+)-([0-9]{8})$/.exec(n);if(!r)return H9().warning(`Latest release tag does not match expected pattern.`,{safe:{tag:n},sensitive:{}}),null;let i=r[2];if(i!==t.archLabel)return H9().info(`Latest release arch does not match current app arch.`,{safe:{tag:n,remoteArch:i,currentArch:t.archLabel},sensitive:{}}),null;let a=Array.isArray(e.assets)?e.assets:[],o=t.assetName.length>0?a.find(e=>e&&typeof e.name==`string`&&e.name===t.assetName):null;o||(o=a.find(e=>e&&typeof e.name==`string`&&e.name.endsWith(`.dmg`)&&e.name.includes(`_${t.archLabel}_`)));if(!o)return H9().warning(`Latest release is missing a matching DMG asset.`,{safe:{tag:n,arch:t.archLabel},sensitive:{}}),null;let s=typeof e.html_url==`string`?e.html_url:null,l=typeof o.browser_download_url==`string`?o.browser_download_url:null;return l==null?(H9().warning(`Latest release DMG asset is missing browser download URL.`,{safe:{tag:n,assetName:o.name??null},sensitive:{}}),null):{tag:n,version:r[1],arch:i,releaseDate:r[3],downloadUrl:l,releasePageUrl:s}}",
    "compareVersionParts(e,t){let n=e.split(`.`).map(e=>Number(e)),r=t.split(`.`).map(e=>Number(e)),i=Math.max(n.length,r.length);for(let e=0;e<i;e+=1){let t=Number.isFinite(n[e])?n[e]:0,i=Number.isFinite(r[e])?r[e]:0;if(t!==i)return t>i?1:-1}return 0}",
    "isRemoteReleaseNewer(e,t){let n=this.compareVersionParts(t.version,e.currentVersion);if(n>0)return!0;if(n<0)return!1;let r=/^[0-9]{8}$/.test(e.currentReleaseDate),i=/^[0-9]{8}$/.test(t.releaseDate);return r&&i?t.releaseDate>e.currentReleaseDate:e.currentReleaseTag.length>0?t.tag!==e.currentReleaseTag:!1}",
  ].join("");
}
